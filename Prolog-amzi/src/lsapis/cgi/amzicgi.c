...
void assertFacts()
{
	...
	while (*p != '\0')
	{
		/* Create a query of the form "cgiAddNewFact(fact, <attribute>, <value>)" */
...
/*	debugFile = fopen("/usr/web/amzi/logs/debug.out", "w"); */
	debugFile = fopen("c:\\WebSite\\cgi-bin\\amzicgi.log", "w");
	...
	while (p != NULL)
	{
		...
		if (val != NULL)
		{
			...
			/* Use atoms for everything except accept, cgi, 
			   extraheaders, use strings for them */
			if (strcmp(envlist[i][2], "accept") == 0 || 
				strcmp(envlist[i][2], "cgi") == 0 ||
				strcmp(envlist[i][2], "extraheaders") == 0)
			{
				doublechar1(val, '$');
				strcat(strbuf, ", $");
				strcat(strbuf, val);
	...
	tf = lsMain(CurEng);	/* Note use lsExecStr if making multiple calls */

	if (tf == FALSE) 
	{
		printf("Content-type: text/plain\n\nFatal Error: lsMain failed\n");
      exitrc = 1;
	}
