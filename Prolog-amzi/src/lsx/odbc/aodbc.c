...
{
   if (msgfile == NULL)
      if ( NULL == (msgfile = fopen("d:\\temp\\lsodbc_log.txt", "w")) )
      {
         return;
      }
...
/*
void errout(eid, char *s)
{
   MessageBoxA(NULL, s, "lsODBC Message", MB_OK);
//   lsErrRaise(g_eid, s);
}
*/
...
void query_del(QUERY* pq)
{
   ...
   else
   {
      ...
      while(px != NULL)
      {
         ...
         errout(g_eid,"Broken LSODBC Query Chain");
      }
   }
   free(pq);
}
...
void query_dump(char* s)
{
   ...
   if (dumpfile == NULL)
      if ( NULL == (dumpfile = fopen("d:\\amzi\\dev\\lsodbc.log", "w")) )
      {
         errout(eid, "Unable to open dump file");
         return;
      }
...
#endif
{
   ...
   if (rc)
   {
      sprintf(buf, "Error #%d Loading aODBC Predicates", rc);
...
//         errout(eid, "success");
      ...
      case SQL_SUCCESS_WITH_INFO:
         if (SQL_NO_DATA_FOUND != SQLError(g_hEnv, g_hDBC, g_hSTMT,
            ...
            errout(g_eid, (char*)szErrorMsg);
         else
            errout(g_eid, "SQL success with info, no additional information available");
         break;
      case SQL_NO_DATA_FOUND:
         errout(g_eid, "no data found"); break;
      case SQL_ERROR:
         strcpy(buf, s);
         strcat(buf, "\n");
         if (SQL_NO_DATA_FOUND != SQLError(g_hEnv, g_hDBC, g_hSTMT,
      ...
      case SQL_INVALID_HANDLE:
         errout(g_eid, "invalid handle"); break;
      case SQL_STILL_EXECUTING:
         errout(g_eid, "still executing"); break;
      case SQL_NEED_DATA:
         errout(g_eid, "need data"); break;
      default:
         errout(g_eid, "something strange"); break;
   }
   return TRUE;
}
...
lsTermToStrQ(eid, tParms, xbuf1, 500);
   ...
   while (OK == lsPopList(eid, &tParms, cTERM, &t))
   {
      ...
      case pFLOAT:
         ...
         break;
      default:
         errout(eid, "Unsupported Prolog type for ODBC input");
      }
      iParm++;
   }
...
char xbuf1[512], xbuf2[512];
   ...
   // Note - if we want to check for NULL data from the SQL query
   // we need to pick up SQL_NULL_DATA from g_ColSize, but this
   // requires more work, as we use one g_ColSize for all, so we
   // need to save a place for each column instead, so we can check
   // it.
   //
   // So what we do instead, is simply fill in our own default
   // values for fields, putting in '' for atoms and strings.
   // Need to add more complete fix for data/time and numbers
   // as well.
   for (pCol = pq->clist; pCol != NULL; pCol = pCol->next)
   {
      ...
      case pdTIME:
         ...
         break;
      default:
         errout(eid, "Unsupported Prolog type for ODBC fetch");
      }
