...
void CWideApp::OnBuildBuild() 
{
   ...
   int rc = LoadString(theApp.m_hInstance, IDS_BUILDER, str_builder, 512);
   pProDoc->SetTitle(str_builder);
   if (pProDoc == NULL) return;
   pProDoc->SetModifiedFlag(FALSE);
   ...
   if (m_pProjectDoc == NULL)
   {
      theCompiler.Run(sFile);
   }
   else
   {
      theCompiler.CompileProject();
   }
...
void CWideApp::OnHelpTibicrules() 
{
   BrowseDoc(_T("tibco\\index.htm"));   
}
