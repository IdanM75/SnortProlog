...
void CDebugView::DoDataExchange(CDataExchange* pDX)
{
   ...
   DDX_Radio(pDX, IDC_FON, m_iFormat);
   //}}AFX_DATA_MAP
}
...
{
   UpdateData();
   theApp.m_pListen->RetractStr(_T("'{sys}leash'('CALL')"));
   if (m_bCall)
      theApp.m_pListen->AssertaStr(_T("'{sys}leash'('CALL')"));
}

void CDebugView::OnLeashExit()
{
   UpdateData();
   theApp.m_pListen->RetractStr(_T("'{sys}leash'('EXIT')"));
   if (m_bExit)
      theApp.m_pListen->AssertaStr(_T("'{sys}leash'('EXIT')"));
}

void CDebugView::OnLeashFail()
{
   UpdateData();
   theApp.m_pListen->RetractStr(_T("'{sys}leash'('FAIL')"));
   if (m_bFail)
      theApp.m_pListen->AssertaStr(_T("'{sys}leash'('FAIL')"));
}

void CDebugView::OnLeashRedo()
{
   UpdateData();
   theApp.m_pListen->RetractStr(_T("'{sys}leash'('REDO')"));
   if (m_bRedo)
      theApp.m_pListen->AssertaStr(_T("'{sys}leash'('REDO')"));
}
...
/*
   ...
   r.bottom -= h/10;
//   ScreenToClient(r);
   this->GetParentFrame()->SetWindowPos(&wndTop, r.left, r.top,
      r.Width(), r.Height(), SWP_DRAWFRAME); 
*/   
